...
function get_icon_for_address(address) {
  ...
  // base it more on the actual filesystem
  if (address === "/") {
    // currently / is our C:\ analog (or C:\Windows\)
...
var go_to = async function (address, action_name = "go") {
  ...
  $("#address-icon").attr(
    "src",
    getIconPath(get_icon_for_address(address), 16)
  );
...
var resolve_address = async function (address) {
  ...
  function handle_url_case() {
    if (
      !address.match(/^https?:\/\/web.archive.org\//) &&
      !address.startsWith(window.location.origin)
    ) {
      ...
      if (address.match(/^https?:\/\/(www\.)?(windows93.net)/)) {
        ...
        !address.match(
          /^https?:\/\/(www\.)?(copy.sh|topotech.github.io\/interdimensionalcable|isaiahodhner.io|brie.fi\/ng)/
        ) &&
      ...
      withFilesystem(function () {
        ...
        fs.stat(address, function (err, stats) {
          ...
          if (stats.isDirectory()) {
            ...
            address =
              window.location.origin + "/" + address.replace(/^\/?/, "");
            is_url = true;
            resolve(handle_url_case());
          }
        });
...
async function render_folder_template(folder_view, address, eventHandlers) {
  ...
  let html = htt.replaceAll(
    percent_var_regexp,
    (match, file_protocol, path_before, var_name, path_after) => {
  ...
  const lowercase_named_color_to_css_var = Object.fromEntries(
    Object.entries(named_color_to_css_var).map(([key, value]) => [
      key.toLowerCase(),
      value,
    ])
  );
  const named_color_regexp = new RegExp(
    `\b(${Object.keys(named_color_to_css_var).join("|")})\b(?!\s*[)?.:=\[])`,
    "gi"
  );
  ...
  $(doc)
    ...
    .each((i, script) => {
      ...
      const func_matches = script.textContent.matchAll(
...
${doc.documentElement.outerHTML}`;
  ...
  const head_end_injected_script_fn = () => {
    ...
    var real_style_descriptor = Reflect.getOwnPropertyDescriptor(
    ...
    getPageStyles().then((css) => {
      css = css.replace(
        /padding:\s*((\d+(?:\.?\d+)?\w+),\s*)/g,
        (match, value) => value.split(/,\s*/g).join(" ")
      );
      css = css.replace(/font:\s*(\d+pt);/, "font-size: $1; line-height: 1;");
      css = css.replace(
        /font:\s*((\d+pt)(\s*\/\s*\d+pt)?) verdana;/,
        "font: $1 'verdana', sans-serif;"
      );
      css = css.replace(
        /style\.(pixel(Width|Height|Left|Top)|left|top)\b/g,
        "styleHack.$1"
      );
      ...
      constructor() {
        ...
        this._params_slot.addEventListener("slotchange", (event) => {
          ...
          for (const param_el of this.querySelectorAll("param, param-hack")) {
            this._params[param_el.getAttribute("name")] =
              param_el.getAttribute("value");
          }
      ...
      connectedCallback() {
        ...
        switch (this.getAttribute("classid")) {
          ...
          case "clsid:1820FED0-473E-11D0-A96C-00C04FD705A2":
            ...
            this.SelectedItems = () => {
              const selected_items = folder_view.items.filter((item) =>
              ...
              GetDetailsOf: (item, detail_id) => {
                ...
                if (item == null) {
                  ...
                  switch (detail_key[detail_id]) {
                    ...
                    case "Modified":
                      return new Date(item._item.resolvedStats?.mtime)
                        .toLocaleString()
          ...
          default:
            console.warn(
              "Unsupported classid value:",
              this.getAttribute("classid"),
              this
            );
            break;
        }
  ...
  html = html.replace(/<object/gi, "<object-hack");
  html = html.replace(/<\/object/gi, "</object-hack");
  html = html.replace(/<param/gi, "<param-hack");
  html = html.replace(/<\/param/gi, "</param-hack");
...
function go_home() {
  go_to("https://web.archive.org/"); // My personal homepage; I might use a search engine, but they don't support iframes
}
  ...
  withFilesystem(function () {
    ...
    fs.stat(file_path, function (err, stats) {
      ...
      if (stats.isDirectory()) {
        ...
        if (frameElement) {
          const systemExecuteFile =
            parent.systemExecuteFile || parent.parent.systemExecuteFile; // dunno if I'll keep the folder view in an iframe
          ...
          alert(
            "Can't open files in standalone mode. Explorer must be run in a desktop."
          );
        }
      }
    });
...
$(function () {
  ...
  $(".back-dropdown-button").on("click", (event) => {
    ...
    const $main_button = $dropdown_button
      .closest(".toolbar-compound-button-wrapper")
      .find(".toolbar-button");
  ...
  function show_history_dropdown(back_or_forward, $main_button, history_stack) {
    const menu_items = history_stack
      .map((path, index) => {
        return {
  ...
  setInterval(() => {
    ...
    $back_history_dropdown_button.attr(
      "disabled",
      can_go_back() ? null : "disabled"
    );
    $forward_history_dropdown_button.attr(
      "disabled",
      can_go_forward() ? null : "disabled"
    );
  }, 100);
  ...
  $(".toolbar-button").each((i, button) => {
    const $button = $(button);
    const sprite_n = [
      0, 1, 44, 21, 22, 23, 24, 26, 31, 38, 0, 1, 2, 3, 4, 5, 6, 12, 7,
    ][i];
    $("<div class='icon'/>")
      .appendTo($button)
      .css({
  ...
  $(document).on("pointerdown", ".toolbar-drag-handle", (event) => {
    ...
    function drag(event) {
      const el_under_pointer = document.elementFromPoint(
        event.clientX,
        event.clientY
      );
      if (!el_under_pointer) {
      ...
      const toolbar_el_under_pointer_prev_sibling =
        toolbar_el_under_pointer.nextElementSibling;
      toolbar_el.parentNode.insertBefore(
        toolbar_el_under_pointer,
        toolbar_el_prev_sibling
      );
      toolbar_el_under_pointer.parentNode.insertBefore(
        toolbar_el,
        toolbar_el_under_pointer_prev_sibling
      );
    ...
    $(".toolbar").each((i, toolbar_el) => {
      ...
      if (toolbar_el.scrollWidth > toolbar_el.clientWidth) {
        if (!$overflow_button.length) {
          $overflow_button = $(
            "<button class='toolbar-overflow-menu-button lightweight'/>"
          )
            .appendTo($toolbar_el)
            .text("»"); // @TODO: SVG
          ...
          $overflow_button.on("click", (event) => {
            ...
            $menu.css({
              ...
              right:
                ...
                $toolbar_el.offset().left -
                $toolbar_el.outerWidth(),
            });
            ...
            const overflowed_items = items.filter(
              (item) =>
            ...
            if ($toolbar_el.is("#menu-bar-toolbar")) {
              ...
              for (const [menu_key, menu] of Object.entries(menus)) {
                if (
                  overflowed_items.some(
                    (item) => item.textContent === remove_hotkey(menu_key)
                  )
                ) {
              ...
              const menu_items = Object.entries(overflow_menus).map(
                ([menu_key, menu]) => {
                  return {
                    item: menu_key,
                    submenu: menu,
