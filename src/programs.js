function show_help(options) {
  ...
  const measure_sidebar_width = () =>
    ...
    parseFloat(
      getComputedStyle($contents[0]).getPropertyValue("margin-right")
    ) +
    $resizer.outerWidth();
  ...
  const $iframe = $(
    "<iframe sandbox='allow-same-origin allow-scripts allow-forms allow-modals allow-popups allow-downloads'>"
  )
  ...
  $resizer.on("pointerdown", (e) => {
    ...
    const getPos = (e) =>
      Math.min(
        $help_window.width() - 100,
        Math.max(20, e.clientX - $help_window.$content.offset().left)
      );
    $G.on(
      "pointermove",
      (pointermove = (e) => {
  ...
  $default_item_li.append(
    make_$item("Welcome to Help").on("click", () => {
  ...
  function renderItemFromContents(source_li, $folder_items_ul) {
    ...
    if ($(source_li).find("li").length > 0) {
      ...
      $item_li.append(
        make_$item(object.Name).on("click", () => {
...
function Notepad(file_path) {
  ...
  var $win = make_iframe_window({
    src:
      "programs/notepad/index.html" + (file_path ? "?path=" + file_path : ""),
  ...
  waitUntil(
    ...
    () => {
      ...
      if (file_path) {
        ...
        systemHooks.readBlobFromHandle(file_path).then(
          (file) => {
            if (file) {
          ...
          (error) => {
            ...
            contentWindow.show_error_message(
...
function SoundRecorder(file_path) {
  ...
  var $win = make_iframe_window({
    src:
      "programs/sound-recorder/index.html" +
...
function showScreensaver(iframeSrc) {
  ...
  $surface.on("mousemove pointermove", (event) => {
    ...
    if (
      Math.hypot(startMouseX - event.pageX, startMouseY - event.pageY) >
      mouseDistanceToExit
    ) {
      cleanUp();
    }
  ...
  var $win = make_iframe_window({
    ...
    constrainRect(rect, x_axis, y_axis) {
      ...
      const new_rect = {
        ...
        width:
          new_inner_rect.width + $win.outerWidth() - $win.$content.outerWidth(),
        height:
          new_inner_rect.height +
          $win.outerHeight() -
          $win.$content.outerHeight(),
      };
      if (x_axis === -1) {
...
function Pinball() {
  ...
  $win.$content.find("iframe").on("game-loaded", () => {
    // custom event dispatched from within the iframe
    $splash.remove();
  });
  $win.$content.find("iframe").on("game-load-failed", () => {
    // custom event dispatched from within the iframe
    $splash.remove();
  ...
  load_winamp_bundle_if_not_loaded(includeButterchurn, function () {
    const webamp_options = {
      ...
      handleTrackDropEvent: (event) =>
        Promise.all(dragging_file_paths.map(filePathToTrack)),
      // TODO: filePickers
    };
    if (includeButterchurn) {
    ...
    webamp.renderWhenReady(visual_container).then(
      () => {
        ...
        winamp_interface.hasClass = (className) => {
          // also for window switcher (@TODO: clean this stuff up)
        ...
        winamp_interface.getTitle = () => {
          ...
          const $cell = $webamp.find(
            ".playlist-track-titles .track-cell.current"
          );
          if ($cell.length) {
        ...
        const animate = () => {
          const windowElements = $(
            ".os-window, .window:not(.gen-window)"
          ).toArray();
      ...
      if (stats.isDirectory()) {
        ...
        if (program) {
          if (!program.acceptsFilePaths) {
            alert(
              program.name +
                " does not support opening files via the virtual filesystem yet"
            );
            return;
          }
          program(file_path);
