...
const system_folder_lowercase_name_to_path = Object.fromEntries(
  Object.entries(system_folder_name_to_path).map(([key, value]) => [
    key.toLowerCase(),
    value,
  ])
);
...
function FolderView(
  ...
  this.cycle_view_mode = () => {
    ...
    const next_view_mode_index =
      (current_view_mode_index + 1) % view_modes.length;
  ...
  this.arrange_icons = () => {
    ...
    if (!self.element.isConnected) {
      // checking parentElement doesn't work if under a shadowRoot
      // console.trace("not in DOM");
      return; // prevent errors computing layout if folder view removed before stats resolve
    }
    const pending_promises = this.items
      .map((item) => item.pendingStatPromise)
      .filter(Boolean);
    ...
    const grid_size_x = large_icons
      ? grid_size_x_for_large_icons
      : grid_size_x_for_small_icons;
    const grid_size_y = large_icons
      ? grid_size_y_for_large_icons
      : grid_size_y_for_small_icons;
    var x = 0;
    var y = 0;
    const dir_ness = (item) =>
      ...
      item.is_system_folder
        ? 2
        : // then folders, then everything else
        item.resolvedStats?.isDirectory()
        ? 1
        : 0;
  ...
  function updateStatus() {
    onStatus?.({
      items: self.items,
      selectedItems: self.items.filter((item) =>
  ...
  self.delete_selected = function () {
    const selected_file_paths = $folder_view
      .find(".desktop-icon.selected")
      .toArray()
      .map((icon_el) => icon_el.dataset.filePath)
      .filter(
        (file_path) => system_folder_path_to_name[file_path] === undefined
      );
    ...
    const name_of_first = $folder_view
      .find(".desktop-icon.selected .title")
      .text()
      .replace(/\.([^.]+)$/, "");
    showMessageBox({
      title:
        selected_file_paths.length === 1
          ? "Confirm File Delete"
      ...
      withFilesystem(function () {
        ...
        if (num_deleted < selected_file_paths.length) {
          alert(
            `Failed to delete ${
              selected_file_paths.length - num_deleted
            } items.`
          );
        }
        // self.refresh();
  ...
  (function () {
    ...
    var drag_update = function () {
      ...
      $folder_view.find(".desktop-icon").each(function (i, folder_view_icon) {
        ...
        folder_view_icon.classList.toggle(
          "selected",
          icon_left < max_x &&
            icon_top < max_y &&
    ...
    $folder_view.on("pointerdown", ".desktop-icon", function (e) {
      ...
      item_el._was_selected_at_pointerdown =
        item_el.classList.contains("selected");
    ...
    $folder_view.on("pointerdown", function (e) {
      ...
      if ($icon.length > 0) {
        ...
        set_dragging_file_paths(
          $(".desktop-icon.selected")
            .get()
            .map((icon) => icon.dataset.filePath)
        ...
        let scrollbar_width =
          $folder_view[0].offsetWidth - $folder_view[0].clientWidth;
        let scrollbar_height =
          $folder_view[0].offsetHeight - $folder_view[0].clientHeight;
        ...
        const over_scrollbar =
          e.clientX > rect.right - scrollbar_width ||
          e.clientY > rect.bottom - scrollbar_height;
      ...
      $($folder_view[0].ownerDocument).on(
        "pointerup blur",
        handle_pointerup_blur
      );
    });
    function handle_pointermove(e) {
      ...
      const scrollbar_width =
        $folder_view.width() - $folder_view[0].clientWidth;
      const scrollbar_height =
        $folder_view.height() - $folder_view[0].clientHeight;
      const clamp_left = $folder_view[0].scrollLeft;
      const clamp_top = $folder_view[0].scrollTop;
      const clamp_right =
        $folder_view.width() + $folder_view[0].scrollLeft - scrollbar_width;
      const clamp_bottom =
        $folder_view.height() + $folder_view[0].scrollTop - scrollbar_height;
    ...
    function handle_pointerup_blur() {
      ...
      $($folder_view[0].ownerDocument).off(
        "pointerup blur",
        handle_pointerup_blur
      );
    }
  ...
  $folder_view.on("keydown", function (e) {
    ...
    if (e.key == "Enter") {
      ...
      if (self.config.view_mode === FolderView.VIEW_MODES.LIST) {
        ...
        for (
          let increment = page_increment;
          increment > 0;
          increment -= item_width
        ) {
      ...
      if (
        (e.ctrlKey || e.metaKey) &&
        $folder_view.find(".desktop-icon.selected").length > 0
      ) {
      ...
      if (search_string === e.key) {
        ...
        const candidates = self.items.filter((item) => {
          ...
          return title
            .toLocaleLowerCase()
            .startsWith(search_string.toLocaleLowerCase());
        });
        if (candidates.length > 0) {
          const index = candidates.findIndex((item) =>
            item.element.classList.contains("focused")
          );
          if (index === -1) {
        ...
        if (e.key !== "Shift" && e.key !== "Compose") {
          // Note: composition doesn't actually work; I'd need an input element to do this properly
          search_string += e.key;
        }
  ...
  function select_item(item_or_item_el, event, delay_scroll) {
    const item_el_to_select =
      item_or_item_el instanceof Element
        ? item_or_item_el
        : item_or_item_el.element;
    const extend_selection = event.shiftKey;
    if (
      selection_anchor_item_el &&
      !self.items.some((item) => item.element === selection_anchor_item_el)
    ) {
    ...
    if (extend_selection && !selection_anchor_item_el) {
      ...
      selection_anchor_item_el =
        self.items.find((item) => item.element.classList.contains("focused"))
          ?.element ?? item_el_to_select;
    }
    ...
    $folder_view.find(".desktop-icon").each(function (i, item_el) {
      if (extend_selection) {
        ...
        const item_el_to_select_rect =
          item_el_to_select.getBoundingClientRect();
          ...
          if (!stats) {
            if (item.pendingStatPromise) {
              try {
                ...
                alert(
                  `Failed to get info about '${item.file_path}':\n\n${error}`
                );
                return;
              }
            } else {
              alert(
                `Cannot open '${item.file_path}'. File type information not available.`
              );
              return;
            }
          }
        ...
        return new Promise(function (resolve, reject) {
          ...
          fs.rename(item.file_path, new_file_path, function (err) {
            ...
            if (item.resolvedStats) {
              const icon_id = icon_id_from_stats_and_path(
                item.resolvedStats,
                new_file_path
              );
